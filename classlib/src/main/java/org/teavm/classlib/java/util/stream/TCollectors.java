...
public final class TCollectors {
    ...
    public static <T, A, R, F> TCollector<T, A, F> collectingAndThen(
            TCollector<T, A, R> downstream, Function<R, F> finisher) {
        return new TCollectorImpl<>(
                downstream.supplier(),
    ...
    public static <T, K> TCollector<T, ?, Map<K, List<T>>> groupingBy(
    ...
    public static <T, K, A, D> TCollector<T, ?, Map<K, D>> groupingBy(
            Function<? super T, ? extends K> classifier, TCollector<? super T, A, D> downstream) {
        return groupingBy(classifier, HashMap::new, downstream);
    }

    public static <T, K, D, A, M extends Map<K, D>> TCollector<T, ?, M> groupingBy(
            Function<? super T, ? extends K> classifier,
            Supplier<M> mapFactory,
            TCollector<? super T, A, D> downstream) {
        return groupingBy0(() -> {
    ...
    private static <T, K, D, A, M extends Map<K, D>> TCollector<T, ?, M> groupingBy0(
        ...
        return TCollector.of(
                ...
                (m1, m2) -> mergeAll(m1, m2, TCollectors::addAll),
                m -> {
...
//      Collector<? super T,A,D> downstream)
    ...
    public static <T, U, A, R> TCollector<T, ?, R> mapping(
            final Function<? super T, ? extends U> mapper, final TCollector<? super U, A, R> downstream) {
        return new TCollectorImpl<>(
                downstream.supplier(),
                (BiConsumer<A, T>)
                        (A a, T t) -> {
    ...
    public static <T> TCollector<T, ?, Optional<T>> minBy(final Comparator<? super T> comparator) {
        return reducing(BinaryOperator.minBy(comparator));
    }

    public static <T> TCollector<T, ?, Map<Boolean, List<T>>> partitioningBy(
            Predicate<? super T> predicate) {
        return partitioningBy(predicate, toList());
    }

    public static <T, D, A> TCollector<T, ?, Map<Boolean, D>> partitioningBy(
            Predicate<? super T> predicate, TCollector<? super T, A, D> downstream) {
    ...
    public static <T, U> TCollector<T, ?, U> reducing(
    ...
    public static <T> TCollector<T, ?, DoubleSummaryStatistics> summarizingDouble(
        ...
        return TCollector.of(
                ...
                TCollector.Characteristics.UNORDERED, TCollector.Characteristics.IDENTITY_FINISH
        );
    }

    public static <T> TCollector<T, ?, IntSummaryStatistics> summarizingInt(
    ...
    public static <T> TCollector<T, ?, LongSummaryStatistics> summarizingLong(
    ...
    public static <T> TCollector<T, ?, Double> summingDouble(final ToDoubleFunction<? super T> mapper) {
    ...
    public static <T, C extends Collection<T>> TCollector<T, ?, C> toCollection(
...
//      BinaryOperator<U> mergeFunction, Supplier<M> mapSupplier)
    ...
    public static <T, K, U> TCollector<T, ?, Map<K, U>> toMap(
    ...
    public static <T, K, U, M extends Map<K, U>> TCollector<T, ?, M> toMap(
            final Function<? super T, ? extends K> keyMapper,
            final Function<? super T, ? extends U> valueMapper,
    ...
    public static <T> TCollector<T, ?, Set<T>> toSet() {
        return TCollector.<T, HashSet<T>, Set<T>>of(
                HashSet::new,
                HashSet::add,
                // TODO switch to a lambda reference once #9333 is fixed
                (c1, c2) -> addAll(c1, c2),
    ...
    private static <T, D, A> D streamAndCollect(TCollector<? super T, A, D> downstream, List<T> list) {
