...
import org.teavm.classlib.java.util.TSpliterators.AbstractDoubleSpliterator;
 ...
 */
final class StreamImpl<T> extends TerminatableStream<StreamImpl<T>> implements TStream<T> {

    /**
     * Represents an empty stream, doing nothing for all methods.
     */
    static class Empty<T> extends TerminatableStream<Empty<T>> implements TStream<T> {
        ...
        @Override
        public TStream<T> sorted(TComparator<? super T> comparator) {
            throwIfTerminated();
            return this;
        }
        ...
        @Override
        public TSpliterator<T> spliterator() {
            terminate();
            return TSpliterators.emptySpliterator();
        }
     ...
     */
    private static final class MapToObjSpliterator<U, T> extends TSpliterators.AbstractSpliterator<T> {
        private final Function<? super U, ? extends T> map;
        private final TSpliterator<U> original;

        public MapToObjSpliterator(Function<? super U, ? extends T> map, TSpliterator<U> original) {
            super(
     ...
     */
    private static final class MapToIntSpliterator<T> extends TSpliterators.AbstractIntSpliterator {
        private final ToIntFunction<? super T> map;
        private final TSpliterator<T> original;

        public MapToIntSpliterator(ToIntFunction<? super T> map, TSpliterator<T> original) {
            super(
                    original.estimateSize(),
                    original.characteristics() & ~(TSpliterator.SORTED | TSpliterator.DISTINCT));
            Objects.requireNonNull(map);
     ...
     * @param <T> the input type
     */
    private static final class MapToLongSpliterator<T> extends TSpliterators.AbstractLongSpliterator {
        private final ToLongFunction<? super T> map;
        private final TSpliterator<T> original;

        public MapToLongSpliterator(ToLongFunction<? super T> map, TSpliterator<T> original) {
            super(
                    original.estimateSize(),
                    original.characteristics() & ~(TSpliterator.SORTED | TSpliterator.DISTINCT));
            Objects.requireNonNull(map);
    ...
    private static final class MapToDoubleSpliterator<T>
            extends TSpliterators.AbstractDoubleSpliterator {
        private final ToDoubleFunction<? super T> map;
        private final TSpliterator<T> original;

        public MapToDoubleSpliterator(ToDoubleFunction<? super T> map, TSpliterator<T> original) {
            super(
                    original.estimateSize(),
                    original.characteristics() & ~(TSpliterator.SORTED | TSpliterator.DISTINCT));
            Objects.requireNonNull(map);
     ...
     */
    private static final class FilterSpliterator<T> extends TSpliterators.AbstractSpliterator<T> {
        private final Predicate<? super T> filter;
        private final TSpliterator<T> original;

        private boolean found;

        public FilterSpliterator(Predicate<? super T> filter, TSpliterator<T> original) {
            super(
                    original.estimateSize(),
     ...
     * @param <T> the type of data to iterate over
     */
    private static final class SkipSpliterator<T> extends TSpliterators.AbstractSpliterator<T> {
        private long skip;
        private final TSpliterator<T> original;

        public SkipSpliterator(long skip, TSpliterator<T> original) {
            super(
     ...
     * @param <T> the type of data to iterate over
     */
    private static final class LimitSpliterator<T> extends TSpliterators.AbstractSpliterator<T> {
        private final long limit;
        private final TSpliterator<T> original;
        private int position;

        public LimitSpliterator(long limit, TSpliterator<T> original) {
            super(
                    original.hasCharacteristics(TSpliterator.SIZED)
    ...
    private final TSpliterator<T> spliterator;

    public StreamImpl(TerminatableStream<?> prev, TSpliterator<T> spliterator) {
        super(prev);
    ...
    @Override
    public <A> A[] toArray(IntFunction<A[]> generator) {
        List<T> collected = collect(TCollectors.toList());
    ...
    @Override
    public <R, A> R collect(final TCollector<? super T, A, R> collector) {
        return collector
                .finisher()
                .apply(
                        reduce(
    ...
    @Override
    public <R> TStream<R> flatMap(final Function<? super T, ? extends TStream<? extends R>> mapper) {
        throwIfTerminated();
        final TSpliterator<? extends TStream<? extends R>> spliteratorOfStreams =
                new MapToObjSpliterator<>(mapper, spliterator);

        AbstractSpliterator<R> flatMapSpliterator =
                new TSpliterators.AbstractSpliterator<R>(Long.MAX_VALUE, 0) {
                    TStream<? extends R> nextStream;
                    TSpliterator<? extends R> next;
    ...
    public TStream<T> sorted() {
        throwIfTerminated();
        TComparator<T> c = (TComparator) TComparator.naturalOrder();
        return sorted(c);
    }

    @Override
    public TStream<T> sorted(final TComparator<? super T> comparator) {
        throwIfTerminated();
        ...
        AbstractSpliterator<T> sortedSpliterator =
                new TSpliterators.AbstractSpliterator<T>(
                    ...
                    TSpliterator<T> ordered;

                    @Override
                    public TComparator<? super T> getComparator() {
                        return comparator == TComparator.naturalOrder() ? null : comparator;
                    }

                    @Override
                    public boolean tryAdvance(Consumer<? super T> action) {
                        if (ordered == null) {
                            TList<T> list = new TArrayList<>();
                            spliterator.forEachRemaining(list::add);
                            TCollections.sort(list, comparator);
    ...
    @Override
    public TStream<T> peek(final Consumer<? super T> action) {
